...
function master(txPorts, minIp, numIps, rate)
	if not txPorts then
		printf("usage: txPort1[,txPort2[,...]] [minIp mindstIp numIPs rate]")
		return
	end
	txPorts = tostring(txPorts)
	minIp = minIp or "10.0.0.1"
	mindstIp = mindstIp or "10.9.3.6"
	...
	for currentTxPort in txPorts:gmatch("(%d+),?") do
		...
		dpdk.launchLua("loadSlave", currentTxPort, 0, minIp, mindstIp, numIps)
	end
	dpdk.waitForSlaves()
end

function loadSlave(port, queue, minA, mindst, numIPs)
	--- parse and check ip addresses

	local minIP, ipv4 = parseIPAddress(minA)
	local mindstIp, ipv4 = parseIPAddress(mindst)
	if minIP then
